...
hdfsFile::hdfsFile(const wstring filename, int fileOptions)
{
#ifdef USE_HDFS
...
size_t hdfsFile::Size()
{
    hdfsFileInfo* fileInfo = hdfsGetPathInfo(m_fs, m_filename);
    if (fileInfo == nullptr)
    {
...
void hdfsFile::SetPosition(uint64_t pos)
{
    if (!CanSeek())
...
void hdfsFile::GetLine(string& str)
{
